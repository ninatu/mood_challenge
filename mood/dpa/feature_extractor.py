...
from mood.dpa.layers import EqualLayer
...
class PretrainedVGG19FeatureExtractor(nn.Module):
    ...
    def forward(self, x, out_keys):
        if len(x) < MAX_BATCH:
            return self._forward(x, out_keys)
        else:
            results = None
            for i in range(0, len(x), MAX_BATCH):
                y = self._forward(x[i: i + MAX_BATCH], out_keys)
                if results is None:
                    results = y
                else:
                    for j in range(len(results)):
                        results[j] = torch.cat((results[j], y[j]), dim=0)
            return results

    def _forward(self, x, out_keys):
        out = {}
